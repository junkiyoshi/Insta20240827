	...
	for (int i = 0; i < 4; i++) {
		...
		for (int len = 300; len > 100; len -= 25) {
			...
			for (int x = -len + size * 0.5; x <= len - size * 0.5; x += size) {
				...
				int height = ofMap(ofNoise(glm::vec4(vertices[0] * 0.005, ofGetFrameNum() * 0.05)), 0, 1, 0, 100 + 300);
				vertices[1].y = height;
				vertices[2].y = height;
				...
				ofColor face_color = z % 50 == 0 ? ofColor(239) : ofColor(39);
				ofColor line_color = z % 50 == 0 ? ofColor(39) : ofColor(239);
